<div class="space-y-2">
    ...
    @foreach ($chats as $chat)
        ...
        @if ($participant instanceof App\Models\User)
            <a href="{{ route('messenger', ['chat' => $chat->uuid]) }}"
                wire:click.prevent="markAsRead('{{ $chat->uuid }}')"
                onclick="window.location.href = this.getAttribute('href')"
                class="block hover:bg-gray-100 rounded-lg transition relative">

                <div class="flex items-center p-3 rounded-lg hover:bg-gray-100 @if ($unreadCount) bg-gray-200 @endif cursor-pointer transition">
                    <div class="relative">
