<div>
    ...
    <div class="h-96 flex flex-col bg-white rounded-lg shadow" wire:poll.30000ms="markMessagesAsRead"
        x-data="{
            init() {
                ...
                <div class="flex items-center p-3 rounded-lg">
                    <div class="relative">
                        @if ($participant->primaryImageView)
                            <img class="h-10 w-10 rounded-full object-cover"
                                src="{{ storage_url($participant->primaryImageView->path) }}" alt="User avatar">
                        @else
                            <img class="h-10 w-10 rounded-full object-cover" src="{{ url('/image/no-image.png') }}"
                                alt="image" />
                        @endif
                    ...
                    @foreach ($messages as $message)
                        <div class="flex {{ $message->user_id == auth()->id() ? 'justify-end' : 'justify-start' }}">
                            <div class="flex flex-col max-w-xs">
                                <div
                                    class="{{ $message->user_id == auth()->id() ? 'bg-blue-100' : 'bg-gray-100' }} rounded-lg p-3">
                                    <p>{{ $message->content }}</p>
                                </div>
                                <p
