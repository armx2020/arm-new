...
- **Разработка**: Replit с MySQL Dev (armbase-2)
- **Production**: Timeweb с MySQL (развертывание через Laravel + GitHub webhook)
...
### Workflow: Replit → GitHub → Timeweb

```
1. Разрабатываете на Replit
2. Пушите изменения в GitHub (git push)
3. GitHub webhook автоматически уведомляет Timeweb
4. Timeweb выполняет deploy.sh и обновляется
```

### Настройка автодеплоя

#### 1. На сервере Timeweb (production):

```bash
# Перейти в папку сайта
cd /var/www/html/arm

# Загрузить последний код с GitHub
git pull origin main

# Сделать deploy.sh исполняемым
chmod +x deploy.sh

# Скопировать DEPLOY_SECRET из .env на Replit
# И добавить в .env на production сервере:
# DEPLOY_SECRET=86e0027869b7032d653def8e2f3a2b7b408e7620e422fbe0b4c59f581f9dcc40
...
2. Заполните:
   - **Payload URL**: `https://vsearmyne.ru/deploy/webhook`
   - **Content type**: `application/json`
   - **Secret**: `86e0027869b7032d653def8e2f3a2b7b408e7620e422fbe0b4c59f581f9dcc40` (из .env)
...
```bash
# На Replit сделайте тестовый коммит
git add .
git commit -m "Test autodeploy"
git push origin main

# GitHub отправит webhook на vsearmyne.ru/deploy/webhook
# Сервер автоматически выполнит git pull и обновится
```

### Что делает deploy.sh:

1. ✅ `git pull` - получает последний код
2. ✅ `composer install` - обновляет зависимости
3. ✅ Очищает кеш Laravel
4. ✅ `php artisan migrate` - выполняет миграции
5. ✅ Кеширует конфигурацию для production
6. ✅ Устанавливает права доступа

### Безопасность:

- ✅ Webhook защищен секретным ключом (DEPLOY_SECRET)
- ✅ GitHub подписывает каждый запрос (X-Hub-Signature-256)
- ✅ DeployController проверяет подпись перед выполнением
- ✅ Только валидные запросы от GitHub выполняются
...
- [x] Настроить автодеплой Replit → GitHub → Timeweb - **ГОТОВО**
  - Создан DeployController для обработки webhook
  - Создан deploy.sh скрипт
  - Добавлен маршрут /deploy/webhook
  - Генерирован DEPLOY_SECRET
- [ ] Настроить GitHub webhook (инструкция готова)
